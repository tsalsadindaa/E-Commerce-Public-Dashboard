...
with st.sidebar:
    start_date = st.date_input(
        label='Start Date', 
        min_value=all_data['order_purchase_timestamp'].min().date(), 
        value=all_data['order_purchase_timestamp'].min().date()
    )
    end_date = st.date_input(
        label='End Date', 
        min_value=start_date, 
        max_value=all_data['order_purchase_timestamp'].max().date(), 
        value=all_data['order_purchase_timestamp'].max().date()
    )
...
with col2:
    total_revenue = format_currency(filtered_data['price'].sum(), "AUD", locale='es_CO') 
